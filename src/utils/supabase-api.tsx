...
export const supabaseApi = {
  ...
  createUserProfile: async (userData: { user_id: string; name: string; avatar_url?: string }) => {
    const { data, error } = await supabase
      .from('profiles')
      .insert(userData)
      .select()
  ...
  updateFamily: async (familyId: string, updates: { name?: string; avatar?: string }) => {
  ...
  getFamilyPosts: async (familyId: string, page = 0, limit = 5) => {
    console.log(`Loading posts for family ${familyId}, page ${page}, limit ${limit}`)
  ...
  createPost: async (postData: { family_id: string; user_id: string; content_text: string; month_tag?: string }) => {
    const { data, error } = await supabase
      .from('posts')
      .insert(postData)
      .select()
  ...
  uploadImage: async (file: File, userId: string) => {
    ...
    try {
      ...
      console.log('Could not list storage buckets:', error)
    }
    ...
    try {
      ...
      if (error) {
        console.warn('Storage bucket not available, using base64 fallback:', error)
        // Use base64 fallback
